...
func handleRPCC() {
  l, e := net.Listen("tcp", "localhost:4002")
    if e != nil {
...
func GrabMessagesFromOtherServers() {
  ...
  var lom ListOfMessages
  lom.ReturnAddress = "localhost:7000"
  b, err := json.Marshal(lom)
  if err != nil {
    fmt.Println(err)
  }
  ...
  serv, _ := rpcc.Dial("tcp", "localhost:4000")
...
func main() {
  ...
  fmt.Println("RUNNING MSG SERVER 1 on port 3002")

  // w := bufio.NewWriter(f)
  go handleRPCC()
  l, _ := net.Listen("tcp", "localhost:3002")
  defer l.Close()
  for {
