...
func (ser *MsgServer) AppendMessages(args *rpcc.ArgToSF, reply *rpcc.ReturnValSF) error {
	...
	rpcc.Logger.LogLocalEvent("Appending-Messages")
...
func GrabMessagesFromOtherServers() {
    ...
    fmt.Println("SLEEPING")
		...
		var lom ListOfMessages
		lom.ReturnAddress = "206.87.208.190:4010"
		b, err := json.Marshal(lom)
		if err != nil {
			fmt.Println(err)
		}
		...
		cfInfo.FileName = "nonLocalCFServer3.go"
		cfInfo.CFName = "1"
		cfInfo.RepoName = "MessageServerCS416"
		cfInfo.ClientIpPort = "206.87.208.190:4010"
		cfInfo.NextIpPort = "206.12.65.114:4001"
		// cfInfo.DebuggingPort = "206.87.208.190:5068"
		var reply rpcc.ReturnValSF
		// address of first hop is 4000
		serv, err := rpcc.Dial("tcp", "206.12.65.114:4001")
    for 
    {
     if	err == nil {
     	break;
...
func ListenForReturnMesages() {
  l, err := net.Listen("tcp", "206.87.208.190:4010")
    if err != nil {
      fmt.Println(err)
      return
    }
...
func main() {
	...
	go rpcc.StartGoVectorPort("client", "206.87.208.190:2700")
	// fmt.Println("RUNNING MSG SERVER 1 on port 3000")

	// w := bufio.NewWriter(f)
	go handleRPCC()
	l, _ := net.Listen("tcp", "206.87.208.190:3000")
	defer l.Close()
	...
	for {
		...
		go func(c net.Conn) {
			...
			for {
				...
				rpcc.Logger.LogLocalEvent("Received-Msg-From-Chat-Client")
				if message != "\n" {
